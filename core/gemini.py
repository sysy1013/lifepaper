...
class _GeminiModel:
    ...
    def _config_without_thinking_config(self) -> genai_types.GenerateContentConfig:
        """thinking_config를 뺀 동일 설정 (사고 끄기를 거부하는 모델용 폴백)."""
        kwargs = {k: v for k, v in self._config_kwargs.items() if k != "thinking_config"}
        return genai_types.GenerateContentConfig(
            system_instruction=self._system_instruction, **kwargs
        )
    ...
    def generate_content(self, prompt: str, model_name: str | None = None):
        try:
            ...
            # 일부 모델은 thinking_budget=0을 거부한다. 그 경우에만 사고 설정을 빼고 1회 재시도.
            if "thinking" not in str(e).lower():
                raise
            return self._client.models.generate_content(
                model=model_name or get_active_model(),
                contents=prompt,
                config=self._config_without_thinking_config(),
            )


# 사고(thinking) 사용 규칙:
# 검출·교정처럼 답이 정해진 작업은 사고를 끄고(속도·비용 이득, 품질 동일),
# 문장을 새로 쓰거나 평가하는 작업은 켠 채로 둔다.
def _make_model(
    api_key: str,
...
9. K-MOOC/MOOC/KOCW, 방과후학교 활동, 연구보고서(소논문) 실적(제목·연구주제·참여인원·소요시간) 언급
   - '소논문'이라는 단어가 전혀 등장하지 않아도, 특정 연구·탐구 활동의 제목(연구주제)을
     참여 인원 수 및/또는 소요 시간과 함께 구체적으로 명시하면 소논문 실적 기재로 보아
     이 기준의 위반으로 판단하라. 키워드 유무가 아니라 서술 내용으로 판단한다.
...
def analyze_with_gemini(text: str, major: str, api_key: str) -> list[dict]:
    ...
    model = _make_model(
        api_key, SYSTEM_PROMPT, temperature=0.2, json_mode=True, thinking=False
    )
...
# ──────────────────────────────────────────────
# Gemini 수행평가 자료 요약 (학생 제출물 → 활동 요약)
# ──────────────────────────────────────────────
ACTIVITY_SUMMARY_SYSTEM_PROMPT = """너는 교사가 세특을 쓸 수 있도록 학생이 제출한 수행평가 자료를 요약해 주는 조수다.

[요약 원칙]
1. 학생이 실제로 수행한 활동, 탐구 과정, 사용한 방법, 만든 산출물, 발견한 내용을 중심으로 정리한다.
2. 4~7개의 짧은 항목으로 쓰고, 각 항목은 '- '로 시작하는 한 줄로 끝맺는다.
3. 자료에 없는 내용을 절대 지어내지 않는다. 추측·평가·칭찬을 쓰지 않고 사실만 적는다.
4. 세특 문체(개조식 어미)로 바꾸지 말고, 교사가 읽고 쓰기 쉬운 담백한 서술로 쓴다.
5. 학생 이름·학번 등이 자료에 있으면 그대로 두고 임의로 지우지 않는다.

[출력 규칙]
- 요약 항목만 출력한다. 제목, 머리말, 마무리 문장, 마크다운 서식을 붙이지 않는다.
"""


def summarize_activity_with_gemini(text: str, api_key: str, subject: str = "") -> str:
    """수행평가 제출물에서 '학생이 실제로 한 일'을 요약한다."""
    model = _make_model(api_key, ACTIVITY_SUMMARY_SYSTEM_PROMPT, temperature=0.3)
    parts = []
    if subject.strip():
        parts.append(f"[과목명]\n{subject.strip()}")
    parts.append(f"[학생이 제출한 수행평가 자료 원문]\n{text}")
    parts.append("위 자료에서 학생이 실제로 한 일을 요약 항목만으로 출력하라.")
    return _gemini_text(model, "\n\n".join(parts)).strip()
...
def proofread_with_gemini(text: str, api_key: str) -> list[dict]:
    ...
    model = _make_model(
        api_key, PROOFREAD_SYSTEM_PROMPT, temperature=0.1, json_mode=True, thinking=False
    )
    parsed = _gemini_json(
